...
| `src/translations/` | One file per locale (`en`, `fr`, `es`, `it`, `de`), registered in `index.ts`. Split out of `localize.ts` so each locale has its own path for `CODEOWNERS` and so parity checks can iterate the registry. |
