...
| `src/utils.ts` | Pure functions: icon mapping, timestamp parsing, `computeAlertProgress()`, severity normalization, zone filtering, alert sorting, `reflowAlertText()`, and the distance helpers `haversineKm()` / `extractPoint()` / `kmToDisplay()` / `displayToKm()`. Operates on `WeatherAlert`. |
...
interface WeatherAlertsCardConfig {
  ...
  maxDistanceKm?: number;      // km from hass.config home point; filters only alerts carrying `point` (point-incident providers) — area warnings never
